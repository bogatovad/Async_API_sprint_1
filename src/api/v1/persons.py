...
async def search_persons(request: Request, person_service: PersonService = Depends(get_person_service))\
    ...
    if not persons:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail=ErrorResponse.PERSON_FILMWORK_NOT_FOUND)
    return [
...
async def person_details(
    ...
    if not person:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail=ErrorResponse.PERSON_NOT_FOUND)
